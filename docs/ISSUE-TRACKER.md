...
| 字段 | 值 |
|------|-----|
| **标题** | 为每个模型新增自定义上游思考参数，未配置时保留现有 enable_thinking 行为 |
| **状态** | 🟢 已修复 |
| **优先级** | P1 |
| **发现日期** | 2026-09-24 |
| **修复日期** | 2026-09-24 |
| **发现人** | 用户需求 |
| **影响范围** | GUI 模型配置；chat_completions 格式且启用 Thinking 的模型 |
| **现象** | 某些上游需要 `chat_template_kwargs.thinking=true` 和 `reasoning_effort=max`，但内置行为固定发送 `enable_thinking=true` |
| **根因** | 请求增强逻辑缺少每个模型的自定义思考参数扩展点 |
| **修复** | 新增 `upstream_thinking_params` 配置字段；GUI 提供 JSON 编辑框并校验 JSON object；配置非空时优先 update 到上游请求，为空时保留默认 `enable_thinking=true` |
| **开发记录** | `docs/dev-notes/2026-09-24-custom-thinking-params.md` |
| **回归测试** | `tests/smoke_test.py`；`tests/test_custom_thinking_params_gui.py`；离屏 GUI 四场景 PASS |
